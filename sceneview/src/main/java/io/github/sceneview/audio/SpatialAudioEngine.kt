...
/**
 * A target the [SpatialAudioEngine] can push a listener pose to. Implemented by
 * [SpatialAudioPlayer]; abstracted as an interface so the engine's registry logic is
 * testable in pure JVM without constructing a JNI-backed `MediaPlayer`.
 */
internal interface AudioListenerTarget {
    @MainThread
...
internal object SpatialAudioEngine {
    ...
    private val players = mutableListOf<AudioListenerTarget>()
    ...
    /** Test hook — number of currently registered players. */
    internal val registeredPlayerCount: Int get() = players.size

    @MainThread
    fun register(player: AudioListenerTarget) {
    ...
    @MainThread
    fun unregister(player: AudioListenerTarget) {
        players.remove(player)
    }

    /**
     * Update the listener pose. Called by [setSpatialAudioListenerPose] each frame.
     *
    ...
    /**
     * Refresh every player using the most recently set listener pose.
     */
    @MainThread
    fun refreshAll() {
        for (i in players.indices) {
    ...
    /**
     * Refresh a single [player] using the most recently set listener pose. Used by
     * [SpatialAudioPlayer] when *its own* world position or volume changes between frames
     * but the listener has not — so we re-apply only that player's gain instead of
     * recomputing the whole registry.
     */
    @MainThread
    fun refreshPlayer(player: AudioListenerTarget) {
        player.updateFromListener(listenerPosition, listenerForward, listenerRight)
    }
...
internal fun panLR(
    ...
    if (length(toSource) < 1e-4f) return 0.5f to 0.5f  // source on top of listener
...
/**
 * Derives the listener's right-vector from its [forward] and [up] basis vectors via a
 * cross product (`right = normalize(forward × up)`). Tolerates non-unit and degenerate
 * input — falls back to `+X` when [forward] and [up] are parallel.
 */
internal fun rightFromForwardUp(forward: Position, up: Position): Position {
    val fwd = Float3(forward.x, forward.y, forward.z)
    val u = Float3(up.x, up.y, up.z)
    val right = cross(fwd, u)
    if (length(right) < 1e-4f) return Position(x = 1f)
    val n = normalize(right)
    return Position(n.x, n.y, n.z)
}
