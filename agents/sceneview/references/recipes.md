...
[`CustomGeometryDemo.kt`](https://github.com/sceneview/sceneview/blob/main/samples/android-demo/src/main/java/io/github/sceneview/demo/demos/CustomGeometryDemo.kt) — generates a torus knot's vertices at runtime and draws them with `MeshNode`. The four steps: build `List<Geometry.Vertex>` (position + normal + UV) in plain Kotlin, build the index list, `Geometry.Builder(primitiveType).vertices(…).indices(…).build(engine)`, then `MeshNode(primitiveType, vertexBuffer, indexBuffer, boundingBox, materialInstance)`. To animate the mesh, `remember` the `Geometry` on the vertex **count** and push new positions with `geometry.update(engine, vertices)` — that rewrites the existing buffers instead of allocating new ones — and free them yourself with `engine.safeDestroyGeometry(geometry)` in a `DisposableEffect`, since `MeshNode` does not own buffers it was handed. `PrimitiveType.LINES` with an edge index list gives you a wireframe of the same vertices.
...
## Recipe: load a World Labs (Marble) world as Gaussian splats (#2646)

[`SplatPreviewDemo.kt`](https://github.com/sceneview/sceneview/blob/main/samples/android-demo/src/main/java/io/github/sceneview/demo/demos/SplatPreviewDemo.kt) is the bundled-asset flow: `context.assets.open(...).readBytes()` off the main thread, `SplatParser.parse(bytes)` on `Dispatchers.Default`, then `SplatNode`. A world generated by World Labs' Marble (the `sceneview-mcp` tool `generate_world`, or the Marble app's `.spz` export) goes through the **same two calls** — there is no World Labs API in the library. Three things differ from a phone scan:

- **Pick the 100k tier on phones.** Marble exports ~2M / 500k / 100k splats; the Android renderer re-sorts on the CPU and re-uploads the cloud while the camera moves, so 500k is beyond a phone's budget. The 100k `.spz` also carries degree-3 spherical harmonics the parser discards (only the degree-0 colour band is kept — colours look a little flatter than in the Marble viewer).
- **Rotate the node 180° about X.** Marble worlds are in the OpenCV convention (+y down, +z forward); Filament is +y up, −z forward. Do it on the node, never by editing the arrays — the painter's sort maps the camera into node space, so sorting stays correct.
- **Have a fallback.** `SplatParser` reads gzip SPZ v2/v3 and rejects v4 (NGSP) with a clear `SplatParseException`; the world's collider mesh (GLB) loads with the ordinary `rememberModelInstance(modelLoader, …)` on every platform — and it is the only option on iOS, which has no splat renderer yet (#2646). The 360° panorama is an SDR equirectangular image: map it onto an inverted `SphereNode` with `materialLoader.createImageInstance(...)`, not the HDR environment loader.

```kotlin
// The 100k .spz bytes — downloaded off the main thread (Dispatchers.IO) and self-hosted:
// World Labs asset links are signed and expire.
