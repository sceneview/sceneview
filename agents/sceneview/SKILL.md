...
- **Android** — `SceneView { … }` (3D) and `ARSceneView { … }` (AR) composables.
  Filament renderer. Artifacts: `io.github.sceneview:sceneview:4.33.0` and
  `io.github.sceneview:arsceneview:4.33.0`.
- **Apple (iOS / macOS / visionOS)** — `SceneView { }` and `ARSceneView { }` SwiftUI
  ...
  via Swift Package Manager (tag `4.33.0`). RealityKit renderer.
