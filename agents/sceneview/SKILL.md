...
- **Android** — `SceneView { … }` (3D) and `ARSceneView { … }` (AR) composables.
  Filament renderer. Artifacts: `io.github.sceneview:sceneview:4.16.4` and
  `io.github.sceneview:arsceneview:4.16.4`.
- **Apple (iOS / macOS / visionOS)** — `SceneView { }` and `ARSceneView { }` SwiftUI
  ...
  via Swift Package Manager (tag `4.16.4`). RealityKit renderer.
- **Web** — `sceneview-web@4.16.4` on npm (Filament.js + WebXR).
- **Flutter** — `sceneview_flutter` plugin (PlatformView bridge).
- **React Native** — `@sceneview-sdk/react-native@4.16.4` (Fabric bridge).
