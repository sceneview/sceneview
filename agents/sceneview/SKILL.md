...
- **Android** — `SceneView { … }` (3D) and `ARSceneView { … }` (AR) composables.
  Filament renderer. Artifacts: `io.github.sceneview:sceneview:4.15.1` and
  `io.github.sceneview:arsceneview:4.15.1`.
- **Apple (iOS / macOS / visionOS)** — `SceneView { }` and `ARSceneView { }` SwiftUI
  ...
  via Swift Package Manager (tag `4.15.1`). RealityKit renderer.
- **Web** — `sceneview-web@4.15.1` on npm (Filament.js + WebXR).
- **Flutter** — `sceneview_flutter` plugin (PlatformView bridge).
- **React Native** — `@sceneview-sdk/react-native@4.15.1` (Fabric bridge).
