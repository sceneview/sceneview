...
- **npm package** — `sceneview-web` (currently `4.38.0`).
...
A world generated by World Labs' Marble (the `sceneview-mcp` tool `generate_world`, or
the Marble app's `.spz` export) loads the same way. Two Marble specifics: prefer the
**500k** tier on the web and the **100k** tier on phones, and rotate the node 180° about
X — Marble worlds are in the OpenCV convention (+y down, +z forward), Filament is +y up,
−z forward:

```js
sv.addSplatNode('https://…/world_500k.spz').then((world) => {
