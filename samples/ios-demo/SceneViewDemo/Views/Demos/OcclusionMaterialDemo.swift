...
/// imperceptible itself.  Paired with a reference model (Damaged Helmet),
/// a flat plane in front of the helmet is given the `OcclusionMaterial`:
/// the result looks like the helmet is partially buried / clipped by
/// invisible geometry.
///
...
struct OcclusionMaterialDemo: View {
    ...
    private var sceneView: some View {
        RealityView { content in
            // ── Reference model (helmet) ──────────────────────────────────
            // A sphere stands in for the helmet since loading async assets
            // requires extra plumbing; the occlusion effect is clearer on
            // a primitive sphere anyway (half-cut look is immediately obvious).
            ...
            let helmetMesh   = MeshResource.generateSphere(radius: 0.25)
            let helmetEntity = ModelEntity(mesh: helmetMesh, materials: [helmetMaterial])
            helmetEntity.name = "helmet"
            helmetEntity.position = [0, 0, -0.7]
            ...
            let occluderMat: Material = showOccluder
                ...
                : OcclusionMaterial()
            let planEntity = ModelEntity(mesh: planeMesh, materials: [occluderMat])
            planEntity.name = "occluder"
            // Position the occluder plane in front of the sphere's lower half.
            planEntity.position = [0, -0.05, -0.58]

            content.add(helmetEntity)
            content.add(planEntity)
        } update: { content in
