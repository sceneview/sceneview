...
- **Mark a breaking change** with a `<!-- breaking -->` line. Major `4` is
  frozen and breaking changes ship as a **minor** bump; a marked (or plainly
  worded) fragment makes the release tooling refuse a patch-level tag, because
  `release.yml` derives the npm version straight from the git tag.
