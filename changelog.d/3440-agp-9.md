...
<!-- RELEASE NOTE (maintainer-only):
     ...
     `:sceneview-compose` moved from `androidTarget()` to AGP 9's `androidLibrary { }`, which
     does NOT create Android host tests by default — Gradle says so out loud ("the 'commonTest'
     source directory exists, but android host tests are not enabled"). No coverage was lost:
     CI never invoked `:sceneview-compose:testDebugUnitTest`, and `commonTest` still runs on the
     desktop JVM and on iOS. Add `withHostTest { }` to the target if the Android JVM run is ever
     wanted back.
