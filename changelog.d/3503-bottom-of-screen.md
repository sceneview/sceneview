...
- **Android demo — the model chooser's call-to-action bar is a container you can see ([#3503](https://github.com/sceneview/sceneview/issues/3503)).** It was `surface` on a `surface` page with no divider, so the catalogue appeared to be sliced by an edge nothing drew. It is now `surfaceContainer` behind a 1 dp `outlineVariant` hairline, and the content padding moved inside the scroll so a card row slides *under* that bar instead of stopping dead against it.
