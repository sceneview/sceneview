...
- The React Native `onTap` "iOS is unverified" caveat pointed readers at
  ...
  the caveat cites it on all six surfaces: `llms.txt`, its
  `website-static/.well-known/` mirror, the regenerated `gpt/knowledge-*`, the
  React Native quickstart, the plugin README and `src/index.tsx` (with the
  `bob`-generated `.d.ts`). The #3072 citations in the README's iOS section and
  in `react-native-sceneview.podspec` are about the podspec gap and are correct;
  they stay.
