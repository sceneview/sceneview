...
- **The React Native surfaces no longer stated as fact that the 3D `onTap` reaches iOS.** That bridge routes its iOS 3D tap through the same `hostView.onTapEntity` hook whose Flutter counterpart was measured never to fire ([#3045](https://github.com/sceneview/sceneview/issues/3045)), so the claim was an untested inference; it was marked unverified pending measurement, deliberately *not* flipped to "Android-only", because asserting the opposite without measuring would repeat the original mistake. That measurement is [#3086](https://github.com/sceneview/sceneview/issues/3086), in this same release: it **does** fire on iOS, and every surface now says so.
