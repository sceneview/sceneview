...
- App Store listing drift is now visible read-only, and the assumption the
  ...
  `sourceFileChecksum` provenance verdict (`confirmed` / `md5-matched` /
  ...
  repo-MD5 match alone reads `md5-matched`; promoting it to `confirmed`
  requires attesting console provenance (`--live-set-is-console-sourced`), so
  uploading our own screenshots can never confirm the assumption by echo
  (#2612).
