...
A **new or resized surface** (first attach, app foregrounded, foldable folded/unfolded, split-screen resize) holds no pixels of its own, so the library always presents into it before parking again — you never have to handle a configuration change yourself. An **async model load** finalises correctly too: `modelLoader.progress < 1f` is one of the tracked sources, because Filament finishes texture uploads from inside the frame loop.
...
**On the web it is the same story — no new call:**
```js
// `sceneview-web` sniffs the payload it fetched, so a .3mf URL goes where a .glb URL goes.
sceneview.modelViewer("canvas", "print.3mf");

// When you hold the BYTES instead — a dropped file, a fetch you made yourself:
const buffer = await file.arrayBuffer();
if (sceneview.isThreeMf(buffer)) {
  const glb = sceneview.threeMfToGlb(buffer);           // Uint8Array, self-contained GLB
  const url = URL.createObjectURL(new Blob([glb], { type: "model/gltf-binary" }));
  sceneview.modelViewer("canvas", url);
}
```
`isThreeMf` never throws; `threeMfToGlb` throws when the payload is not a readable 3MF. Both take an
`ArrayBuffer` or any `ArrayBufferView`.
...
// 3MF (#3482) — a `.3mf` URL needs nothing extra; these are for raw bytes.
sceneview.isThreeMf(bytes)      // ArrayBuffer | ArrayBufferView -> boolean, never throws
sceneview.threeMfToGlb(bytes)   // -> Uint8Array (GLB); throws if not a readable 3MF
