...
measure against); a tap that hits nothing trackable must fall back to the
screen-space card; and with the composited capture above, hide the anchored
cards during the capture (`isVisible = false`) or the model reads its own
earlier answers back as part of the next question.
