...
/// ``onTapModel`` (both publish the tapped node's name, and it is public API on each) and
/// ``SceneViewerConfiguration/cameraPoseAuthored`` (neither has a camera at all) are all
/// there because a bridge would otherwise have regressed silently.
///
/// ### Using it
///
/// ```swift
...
public final class SceneViewerHostView: UIView {
    ...
    /// Called after a tap, with the **model** that was tapped — or `nil` when the tap
    /// landed on nothing this host loaded.
    ///
    /// The Swift-only counterpart to ``onTap``, which flattens a hit into five primitives
    /// because it has to cross into Kotlin. This one hands over a ``SceneTapHit`` whose
    /// `entity` is the model root, so `entity.name` is the model's name — the one derived
    /// by ``sceneViewerNodeName(explicit:request:)``, never a name from inside the asset.
    ///
    /// **That resolution is the point of this callback.** `SpatialTapGesture` reports the
    /// deepest hit entity and USD assets routinely name their meshes, so the raw hit is a
    /// sub-mesh: a tap anywhere on `black_dragon.usdz` reported `skin0`. Resolving here
    /// rather than in each bridge is what makes "a tap reports the model file's base name"
    /// one contract with one implementation across every caller of this host, and what
    /// makes it match what Android reports for the same tap — on Android only the
    /// `ModelNode` root owns a collider, so a hit can never be asset-internal geometry.
    ///
    /// `hit.worldPosition` is deliberately **not** re-derived from the model root: it stays
    /// the bounds centre of the geometry the finger actually landed on, which tracks the
    /// tapped part of a multi-mesh model and is as close as this platform gets to Android's
    /// ray-surface hit point. See ``SceneTapHit``.
    ///
    /// Both callbacks can be set at once and both fire; ``onTap`` fires for every tap,
    /// including one this reports as `nil`. Not `@objc`: ``SceneTapHit`` carries RealityKit
    /// types that do not cross into Objective-C, and the consumers that need it — the
    /// Flutter and React Native bridges — are Swift.
    public var onTapModel: ((SceneTapHit?) -> Void)? {
        get { state.onTapModel }
        set { state.onTapModel = newValue }
    }
...
final class SceneViewerState: ObservableObject {
    ...
    var onTapModel: ((SceneTapHit?) -> Void)?
    ...
    /// - Parameter model: the model root `hit` resolved to, or `nil` when the tap landed
    ///   on nothing this host loaded. Resolved by the caller, which is the only place that
    ///   holds the content root.
    func reportTap(_ hit: SceneTapHit, model: Entity?) {
        ...
        //
        // The position is carried over from the raw hit rather than re-derived from the
        // model root — see `SceneViewerHostView.onTapModel` for why.
        onTapModel?(model.map { SceneTapHit(entity: $0, worldPosition: hit.worldPosition) })
...
struct SceneViewerRootView: View {
    ...
    var body: some View {
        ...
        .onEntityTapHit { hit in
            state.reportTap(hit, model: scene.contentRoot.modelRoot(for: hit.entity))
        }
