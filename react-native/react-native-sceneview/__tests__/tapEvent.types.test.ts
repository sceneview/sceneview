 ...
 * `SceneViewManager` and `ARSceneViewManager`), the iOS 3D wrapper (a payload
 * dict seeded with `"nodeName": NSNull()` and only ever overwritten), and the
 * iOS AR wrapper (a literal carrying `"nodeName": NSNull()`). So a consumer
 * needs one `nodeName == null` guard, not a two-sentinel `null`/`undefined`
 * one.
 *
...
type Exact<T, U> = (<G>() => G extends T ? 1 : 2) extends <G>() => G extends U
  ? 1
  : 2
  ? true
  : false;
...
type NodeNameIsExactlyStringOrNull = AssertTrue<
  Exact<TapEvent['nodeName'], string | null>
>;
...
describe('TapEvent.nodeName', () => {
  it('is a single null sentinel across every dispatch path', () => {
    ...
    const _typeCheck: NodeNameIsExactlyStringOrNull = true;
