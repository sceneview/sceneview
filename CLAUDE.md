...
- Use `SceneView { }` for 3D-only scenes (`io.github.sceneview:sceneview:4.16.10`)
- Use `ARSceneView { }` for augmented reality (`io.github.sceneview:arsceneview:4.16.10`)
...
### Current state (last updated: 2026-05-28, session pedantic-robinson — maintenance #4 + perf-umbrella contributions + batch-review)
...
- 🔧 **macOS App Store CI — root cause found & gated** (#2252, reopened): deploy needs TWO certs (Apple Distribution → `.app`, Mac Installer → `.pkg`). The Apple Distribution cert Xcode auto-generated is **non-extractable** (`kSecAttrIsExtractable=0`, ACL allows only `identityservicesd`) — verified via `security dump-keychain` AND Keychain Access GUI (Export menu disabled, confirmed via AppleScript). `app-store.yml` now gates `macos_ready` on `MACOS_APP_CERT_BASE64` (skips cleanly until set) and imports BOTH certs. Fix needs a CSR-regenerated Apple Distribution cert — full 10-step walkthrough in #2252.
- ⬆️ **Filament 1.71.4 → 1.71.5 SHIPPED to main** — patch bump (no MATERIAL_VERSION change, no `.filamat` recompile); compile + tests green, CI green.
- ⚙️ **Perf umbrella #2263 contributions (mine, both green/mergeable, awaiting Thomas's merge):**
  - **PR #2289** (#2265) — pre-decomposed TRS-tuple `slerp` overload in `sceneview-core` + `NodeAnimationDelegate.onFrame` single-read fed by #2187-cached components (3 JNI reads→1, 6 decomps→3). Resolved a post-merge conflict with #2280 (which also landed an `onFrame` single-read); kept the superset. CI green.
  - **PR #2291** (#2270) — batch ARCore per-frame updated-trackables/anchors/planes (O(N×M)→O(N+M)), API-safe via internal per-frame sets, delegated to a background Opus agent + diff-verified. CI green, conflict-free.
- ✅ **Batch-reviewed 4 non-mine perf PRs** (Thomas-requested): #2286 MERGE, #2281 MERGE-WITH-NITS, #2282 MERGE-WITH-NITS, #2280 MERGE-WITH-NITS (+ flagged the #2280↔#2289 `onFrame` overlap on the PR). **#2280/#2286/#2282/#2281 ALL merged to main** + demo Batch 2 (#2290).
- 🔬 **Perf umbrella #2263 ORIGIN = the hot-path audit session** (vibrant-saha): 5-surface Opus audit → 83 findings (22 HIGH) → umbrella #2263 + 14 sub-issues #2264–#2278. Authored & merged #2280 (HP-1 world-cache + HP-6 transformInstance + HP-2 partial), #2282 (HP-8 Mat4.copyColumnsInto), #2286 (HP-12 Ray refs), #2281 (HP-9 Manipulator). Ran an **independent review wave** that caught + fixed 2 real blockers + 1 residual stop-frame stale-read in #2280's world-cache (animated glTF sub-nodes / ModelNode-move propagation) — invisible to unit tests. HP-10 (#2273) reverted (valid→empty Filament crash risk; safe redesign documented).
- 🆕 **Follow-up issues filed**: #2283 (entityDragGesture cumulative-vs-delta correctness bug), #2284 (Filament-backed world-cache regression test), #2285 (renderableInstance/lightInstance cache), #2287 (PlaneVisualizer per-frame getInstance).
- ✅ **sync-versions 0 MISMATCH** (3 expected WARNs), main compile + unit tests green post-merge, AI skills ✅, website OK.

### Followups for next session

1. **MERGE the open perf PRs** — mine: **#2289** (slerp, green/mergeable), **#2291** (AR batching, green/conflict-free). (#2281 Manipulator cache already MERGED.) Then the remaining perf-umbrella children are unclaimed and ready: **#2266** (AR PoseNode setter — AR-side #2187 cousin), **#2267** (worldToLocalQuaternion — NOW unblocked, #2264 merged; a rate-limited agent attempt produced nothing, issue still open), **#2268** + **#2274** (web), **#2277**+**#2278** (SwiftUI — same file, do as ONE PR). NOTE: before claiming a perf issue, check existing open PRs' **diffs** (not just issue links) — #2280 had quietly folded #2265's single-read into its scope, causing the #2289 conflict.
2. **[#2252](https://github.com/sceneview/sceneview/issues/2252) macOS Apple Distribution cert (non-extractable)** — Thomas action: regenerate via Keychain Access → Certificate Assistant CSR flow (keeps key extractable), revoke old cert `3C376942…`, re-link the "SceneView Demo Mac App Store" profile, then set `MACOS_APP_CERT_BASE64` + `MACOS_APP_CERT_PASSWORD` + refresh `MACOS_PROVISIONING_PROFILE_BASE64`. Full walkthrough in the issue.
...
4. **[#1831](https://github.com/sceneview/sceneview/issues/1831)** — stale `appStoreVersionSubmission` auto-delete before POST in `app-store.yml` (self-contained CI fix, in-domain, non-colliding — good next pickup).
5. **[#2241](https://github.com/sceneview/sceneview/issues/2241) Sprint 1 V2 plane renderer redo** — PlaneDiscoveryGuide + ShadowReceiverPlane + PlacementReticle + ARPlacementDemo (RELIRE `project_v2_plane_renderer_lessons.md` first).
6. **[#2239](https://github.com/sceneview/sceneview/issues/2239) Samples catalog regrouping** — Batch 1+2 merged; more batches in flight from parallel sessions.
