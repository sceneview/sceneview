...
- Use `SceneView { }` for 3D-only scenes (`io.github.sceneview:sceneview:4.17.0`)
- Use `ARSceneView { }` for augmented reality (`io.github.sceneview:arsceneview:4.17.0`)
...
### Current state (last updated: 2026-05-31 — **v4.17.0 RELEASED** (hot-path/perf + complete #2187 fix))

- 🚀 **v4.17.0 TAGGED & PUBLISHING** — the #2263 hot-path/perf umbrella + the **complete #2187 fix** (#2337, component setters no longer re-decompose) + Engine-backed regression test tier (#2334/#2337/#2339) + iOS drag-delta fix (#2312) + hot-path API-guidance doc (#2314). Minor bump from 4.16.10 (new public APIs additive: `Mat4.copyColumnsInto`, slerp TRS overload, `Pose.toTransform(out)`; major 4 frozen). **VERIFIED LIVE:** npm `sceneview-web@4.17.0` ✅, GitHub Release v4.17.0 ✅ (thematic body + demo APKs). **In flight (verify before claiming live):** Maven Central (release.yml success → repo1 propagation pending), Play Store (deploy queued), iOS App Store (deploy workflow **success** — the #2252 cert fix held — but store-live is review-gated, not yet live), website (deployed ✅). `release-checklist` gate: 1 "blocker" was a **false positive** (`sk-` substring matching "disk-full" — fixed the regex to real key shapes) → RELEASE POSSIBLE with 3 advisory WARNs. mcp stays on its independent track (4.0.12).
...
2. **[#2301](https://github.com/sceneview/sceneview/issues/2301) reviewSubmissions stale-open robustness** — the #1831 class can recur on the v4 API (CI fix, self-contained).
