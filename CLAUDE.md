...
| `store-sync/asc_listing.py` | App Store listing drift check, READ-ONLY (#2612 P2 Phase A) — diffs live ASC text fields + screenshot `sourceFileChecksum` (MD5) against `samples/ios-demo/distribution/app-store/` + `appstore-screenshots/`. Upload half = Phase B (an `--apply*` flag errors loudly until then). Same ASC env aliases as `store-preflight.sh` |
