...
| **Saved workflows** (`.claude/workflows/*.js`) | Multi-agent orchestration with deterministic, **resumable** control-flow | `triptych.js`, `fix-issue-batch.js`, `review-fanout.js`, `parity-audit.js`, `audit-sweep.js`, `release-checkpoint.js` |
...
| `parity-audit.js` | LIVE | periodic / post-API-change | per target platform (iOS/Web/bridges) one auditor ∥ lists gaps vs the Android reference → adversarially verify each gap against source (honest "Coming soon" excluded) → `GAPS_CONFIRMED`/`PARITY_CLEAN`/`AUDIT_INCOMPLETE` |
