...
# Resolve with a controlled environment: no PATH node, no Homebrew node, and a
# HOME pointing at the fixture. Runs in a subshell so the caller's env survives.
resolve_in() {
    ...
    (
        ...
        # An empty PATH would break `find`/`sed`/`sort` inside the lib, so keep
        # the system dirs and rely on this host not having node in them. If it
        # does, the PATH case is exercised instead — asserted separately below.
