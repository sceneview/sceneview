...
if [ -f "$LLMS" ] && [ -f "$DOCS_LLMS" ]; then
    if ! diff -q "$LLMS" "$DOCS_LLMS" >/dev/null 2>&1; then
        ...
        if [ "$FIX_MODE" = "--fix" ]; then
            ...
            # the remaining checks (e.g. the MCP bundle drift check below).
...
# ─── 5b-bis. MCP generated bundle drift (issue #1808) ─────────────────────
# `mcp/src/generated/llms-txt.ts` is committed to git so the gateway (which
# has no FS) and the npm package both ship the same byte-identical bundle.
# It's a checked-in artefact regenerated from root `llms.txt` by
# `mcp/scripts/generate-llms-txt.js`. Sprint features (e.g. DepthMeshNode,
# Future-returning Cloud Anchor APIs) repeatedly landed in `llms.txt`
# without regenerating the bundle, so the MCP server kept serving a stale
# API surface to AI clients. This check rebuilds the bundle into a temp
# file and compares — no destructive write. `--fix` runs the real
# generator to update the committed file in place.
MCP_BUNDLE="$REPO_ROOT/mcp/src/generated/llms-txt.ts"
MCP_GEN_SCRIPT="$REPO_ROOT/mcp/scripts/generate-llms-txt.js"
if [ -f "$LLMS" ] && [ -f "$MCP_BUNDLE" ] && [ -f "$MCP_GEN_SCRIPT" ] && command -v node >/dev/null 2>&1; then
    # Write the expected bundle to a temp file (not `$(...)` substitution —
    # bash strips trailing newlines on command substitution, but the real
    # bundle ends with `;\n` so a string-based diff was always reporting
    # drift by one trailing byte).
    ...
    if [ -n "$EXPECTED_BUNDLE_FILE" ]; then
        ...
        ' "$LLMS" "$EXPECTED_BUNDLE_FILE" 2>/dev/null; then
            if ! diff -q "$EXPECTED_BUNDLE_FILE" "$MCP_BUNDLE" >/dev/null 2>&1; then
                if [ "$FIX_MODE" = "--fix" ]; then
                    ( cd "$REPO_ROOT/mcp" && node scripts/generate-llms-txt.js >/dev/null 2>&1 ) || true
                    echo -e "${GREEN}  Fixed: regenerated mcp/src/generated/llms-txt.ts from root llms.txt${NC}"
                else
