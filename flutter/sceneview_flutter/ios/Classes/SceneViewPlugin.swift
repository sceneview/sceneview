...
class SceneViewPlatformView: NSObject, FlutterPlatformView {
    ...
    init(frame: CGRect, viewId: Int64, args: [String: Any], messenger: FlutterBinaryMessenger) {
        ...
        //
        // `onTapModel` rather than the host's flattened `onTap`: what Dart
        // receives is the tapped model's name, and the flattened callback
        // carries a position, not an entity to read a name from. The host
        // resolves the raw RealityKit hit — which is the deepest entity, so
        // routinely a sub-mesh — up to the model root and names that root after
        // its file, so this bridge no longer walks the tree itself.
        //
        // `nil` means the tap hit no bridge-loaded model. Dart's `onTap` takes a
        // non-null String, so that case sends `""` — the value this bridge has
        // always sent for a tap that resolved to no model.
        let channel = self.channel
        MainActor.assumeIsolated {
            hostView.autoresizingMask = [.flexibleWidth, .flexibleHeight]
            hostView.onTapModel = { [weak channel] hit in
                channel?.invokeMethod("onTap", arguments: hit?.entity.name ?? "")
            }
